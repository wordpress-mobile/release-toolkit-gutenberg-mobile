...
version_file="Podfile"
test -f "$version_file" || abort "Error: Could not find $version_file"
sed -i'.orig' -E "s/wordpress-mobile(\/gutenberg-mobile)/$MOBILE_REPO\1/" $version_file || abort "Error: Failed updating GitHub organization in $version_file"
sed -i'.orig' -E "s/gutenberg (commit|tag): '(.*)'/gutenberg commit: '$GB_MOBILE_PR_REF'/" $version_file || abort "Error: Failed updating gutenberg ref in $version_file (new format)"
